...
#
#  This code is provided to facilitate you in developing your code, runing and
#  testing your code. You can use any of the functions and variables defined in
#  this program via objects of these classes. BUT, you cannot use any of the following
#  three protected variables of class Cave in any way in your code:
#
...
#  in addition to the required function probability_based_move(). In this case, you need to
#  link these additional functions at the beginning of class Robot. 
#  
...
# the locations of the wumpus, pit, and gold chosen by the user for the fixed board
wumpus_column,wumpus_row, pit_column,pit_row, gold_column, gold_row = 0,0,0,0,0,0
...
# ---------------------------------------------------------------------------------------------
    ...
    def __init__(self,cave):
        ...
        self.available_rooms=set() # A set of room locations currently available, but may not be accessible to the robot. 
        ...
        self.path = [] # A list of room locations that have been visited by the robot so far. The location at position 0 is the first room that the robot has visited.
                       # It may contain duplicates, which means some of the locations may have been visited more than one time.
        ...
        self.jdP_PWs = self.PitWumpus_probability_distribution(self.cave.WIDTH, self.cave.HEIGHT)
        # The function PitWumpus_probability_distribution() is to build the joint probability distribution of pits/wumpus in the given cave environment.
        # The function return the joint probability distribution.
        # The function PitWumpus_probability_distribution()is defined in probability_based_move.py
        # self.jdP_PWs: an object of JointProbDist
...
#---------------------------------------------------------------------------------------------------
    ...
    def move(self, column, row):
        ...
        if (column,row) not in self.visited_rooms:
            ...
            ## Call method kb_add() to add logic expressions about perceived stenchs and breezes to the knowledge base
...
#---------------------------------------------------------------------------------------------------       
    def step(self, agent_type, max_prob):
        ...
        # Assign the maximum probability threshold to variable max_pit_probability
        self.max_pit_probability = max_prob

        # Two functions can be used to determine the next room: next_room() and next_room_prob()
        # If agent_type = 0, function next_room() will be called. It is an existing function 
        # included in the provided program. This function finds a safe room by using the logic
        # resolution method.
        #
...
#---------------------------------------------------------------------------------------------------
    ...
    def has_breeze(self, column, row):
        ...
        if (column,row) in self.visited_rooms or (column==self.current_position[0] and row == self.current_position[1]):
            ...
            messagebox.showinfo("Errors", "The position has not been visited yet.")
            return False
...
#---------------------------------------------------------------------------------------------------
    def observation_pits(self, observed_locations):
        ...
        # Since the visited rooms must have no pit/wumpus, the truth value should be false.
        ...
        for each_v in observed_locations:
            ...
            else: messagebox.showinfo("Errors", "The position has not been visited yet.")
        return known_PW
...
#---------------------------------------------------------------------------------------------------
    def consistent(self, known_BS, event):
        ...
        # event: a dictionary containing all rooms with corresponding truth value for pits
        #        Each element in event is a pair of variable name and truth value, var:val, e.g. {'(1,2)':True, '(2,3)':False}.
        # This function is to check if the known values in known_BS are consistent with the values in the event.  
...
#---------------------------------------------------------------------------------------------------                
    def surrounding_contain_pits(self, column, row, event):
        ...
        for (col, ro) in surrounding: # each element is a pair of (column, row)
            ...
            if var_name in event.keys() and event[var_name]==True: # only check the rooms which are in event
...
#---------------------------------------------------------------------------------------------------                
    def check_safety(self, column, row):
        ...
        # Define the sentence to be proved
        wumpus_expr = expr("~W%d%d" %(column,row)) # no wumpus at (column, row) 
        pit_expr = expr("~P%d%d" %(column,row)) #no pit at (column, row) 
       
        return pl_resolution(self.kb,pit_expr) and pl_resolution(self.kb,wumpus_expr)
...
class GUI(Cave):
    ...
    def __init__(self,parent,cave,robot):
        ...
        # Add the agent types
        self.agent_type['values'] = ('Logic based agent', 'Probability based agent')
        ...
        self.var = DoubleVar(value = 0.0)
        self.max_prob = Spinbox(self.game_setting, from_=0.0,to =1.0,increment=0.1, textvariable=self.var)
        ...
        # Create quit button  
        self.button_quit=Button(self.parent,text=" Quit Game ",command=self.quit_game)
    ...
    def start(self):
        ...
        if self.robot.gameover == False:
            ...
            if self.robot.current_position == self.cave._goldCoor:               
                ...
                messagebox.showinfo("Congrats","You have found the gold and you won!")
            ...
            if self.robot.dead == True:
                ...
                messagebox.showinfo("Game over","You dead!")
            else:
                ## Get the number of moves and set into the result frame
                self.s_move_num.set(str(self.robot.num_moves-1))
                ## Get the result and set into the result frame
                self.s_success.set('No rooms!')
                messagebox.showinfo("Game over","No available rooms to explore, fail!")
...
## Get the board settings that the user has chosen and start a new game 
def getInfor():
    global fixed_board, wumpus_column,wumpus_row, pit_column,pit_row, gold_column,gold_row
    
    ## Get the boarding setting 
    number_of_rows=int(w1.get())
    number_of_columns=int(w2.get())
    number_of_pits = int(w3.get())

    if b.get() == 1: # Fixed board
        wumpus_row  = int(w_row.get())
        wumpus_column = int(w_column.get())
        pit_row  = int(p_row.get())
        pit_column = int(p_column.get())
        gold_row  = int(g_row.get())
        gold_column = int(g_column.get())
        
        fixed_board = True
        if (wumpus_column,wumpus_row) in {(1,3),(1,2),(2,3)} or \
           (pit_column,pit_column) in {(1,3),(1,2),(2,3)} or\
           (gold_column,gold_row)==(1,3) or\
           (wumpus_row==pit_row and wumpus_column==pit_column) or\
           (wumpus_row==gold_row and wumpus_column==gold_column) or\
           (gold_row==pit_row and gold_column==pit_column):
            messagebox.showinfo("Wrong setting!",  "The wumpus, pit and gold can't occupy the same room and can't be in the 3 specific rooms. Do it again!")
            return
    else:
        fixed_board = False
        
    ## Destroy the setting panels
    setting_up.destroy()
    board.destroy()
    ## Start a new game
...
def newGame(main_GUI, number_of_columns,number_of_rows,number_of_pits):
    ...
    if fixed_board == True:
        cave  = Cave(number_of_columns,number_of_rows,number_of_pits, fixed_board, wumpus_column,wumpus_row, pit_column,pit_row, gold_column,gold_row)
    else:
...
if __name__ == '__main__':
    ...
    Label(setting_up,text = "Board Setting", font = ('Arial', 14, 'bold')).\
          grid(row = 0, column = 0, columnspan = 2,padx = 5, pady = 5) 

    # Let user choose the number of rows in the cave environment
    rows = Label(setting_up,text="Please choose number of rows:")\
           .grid(row = 1, column = 0,padx = 5, pady = 5)
    var = DoubleVar(value=3)
    w1 = Spinbox(setting_up, from_=3,to =4, textvariable=var)
    w1.grid(row = 1, column = 1,padx = 5, pady = 5)
    
    # Let user choose the number of columns in the cave environment
    columns =Label(setting_up,text="Please choose number of colums:")
    columns.grid(row = 2, column = 0,padx = 5, pady = 5)
    var1 = DoubleVar(value=3)
    w2= Spinbox(setting_up,from_=3,to =4, textvariable=var1)
    w2.grid(row = 2, column = 1,padx = 5, pady = 5)

    # Let user choose the number of pits in the cave environment
    pits =Label(setting_up,text="Please choose number of pits")
    pits.grid(row = 3, column = 0,padx = 5, pady = 5)
    var2 = DoubleVar(value=1)
    w3= Spinbox(setting_up,from_=1,to =3, textvariable=var2)
    ...
    random = Radiobutton(setting_up, text="Random Board", variable=b, value=2)
    random.grid(row=4, column = 1, padx = 5, pady = 5)
...
three must occupy different rooms.', \
          font = ('Arial', 10)).grid(row = 1, column = 0, columnspan = 4,padx = 5, pady = 5)
    ...
    button = Button(setting_up,text="Set Up",  command=getInfor).grid(row = 6, column = 0, columnspan = 2 )

    ## Set up the environment board, here it is an empty area, just a place holder
    board=Canvas(main_GUI,width=board_width,height=board_height,highlightbackground='black')
